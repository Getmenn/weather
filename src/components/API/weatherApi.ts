...
export const weatherApi:WeatherApi = {
    getWeatherNow: async (lon, lat) => { //погода сейчас
        const response: AxiosResponse<any> = await weather.get(`weather?lat=${lat}&lon=${lon}&appid=${apiKey}`);
        console.log(response.data);
        
        //return coord;  
    },
    geWeatherFiveDay: async (lon, lat) => { //погода на 5 дней
        const response: AxiosResponse<any> = await weather.get(`forecast?lat=${lat}&lon=${lon}&appid=${apiKey}`); 
        console.log(response.data);
    },
