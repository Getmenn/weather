...
interface WeatherApi {
    ...
    getWeatherNowFromCity: (city: string) => Promise<any>;
}
...
export const weatherApi:WeatherApi = {
    ...
    getWeatherNowFromCity: async (city) => { //координаты города //не нужно
        const response: AxiosResponse<any> = await weather.get(`/weather?q=${city}&appid=${apiKey}&lang=ru&units=metric`);
        console.log(response.data); 
        return response.data
    },
    ...
    getCoordCity: async (city) => { //координаты города //не нужно
